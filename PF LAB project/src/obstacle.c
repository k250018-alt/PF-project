...
void obstacle_manager_spawn(ObstacleManager *manager, gint width, gint height, GdkPixbuf *sprite) {
    ...
    if (manager->spawn_timer <= 0) {
        ...
        Obstacle *obstacle = obstacle_new(x, -h - 10, w, h, vel, sprite);
...
void obstacle_manager_draw(ObstacleManager *manager, cairo_t *cr, GdkPixbuf *sprite) {
    ...
    for (guint i = 0; i < manager->obstacles->len; i++) {
        Obstacle *obstacle = g_ptr_array_index(manager->obstacles, i);
        if (sprite) {
            graphics_draw_pixbuf(cr, sprite, obstacle->x, obstacle->y, obstacle->width, obstacle->height);
        } else {
...
void obstacle_manager_free(ObstacleManager *manager) {
    ...
    if (manager->sprite_template) {
        g_object_unref(manager->sprite_template);
    }
    g_free(manager);
}
